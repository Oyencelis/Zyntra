...
BEGIN
  ...
  IF v_status = 8 THEN
    ...
    SET qty = COALESCE(p.qty, 0) + restored.quantity,
        updated_at = NOW()
    FROM (
      SELECT oi.product_id,
             SUM(GREATEST(COALESCE(oi.quantity, 0), 0))::integer AS quantity
      FROM public.order_items AS oi
      WHERE oi.suborder_id = p_suborder_id
        AND oi.status IN (1, 7)
      GROUP BY oi.product_id
    ) AS restored
    WHERE p.product_id = restored.product_id
...
BEGIN
  ...
  IF v_status = 8 AND v_product_id IS NOT NULL AND v_quantity > 0 THEN
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'pickup_location', r.seller_location,
        'buyer_id', r.buyer_id,
...
BEGIN
  ...
  IF p_status = 4 THEN
    ...
    IF COALESCE(v_commission_amount, 0) > 0 THEN
      ...
      SELECT v_user_id, 'rider', ROUND(v_commission_amount, 2), 'rider_commission_delivery_item', p_order_item_id, 'Rider delivery commission and convenience fee share'
      WHERE NOT EXISTS (
        SELECT 1
        FROM public.wallet_ledger AS wl
        WHERE wl.user_id = v_user_id
...
BEGIN
  ...
  FROM (
    SELECT n.order_id, n.title, n.message, n.created_at
    FROM public.notifications AS n
    WHERE n.user_id = v_user_id
...
DROP POLICY IF EXISTS "Allow authenticated users to update own notifications" ON public.notifications;
...
DECLARE
  ...
  v_seller_item_preview text := '';
  v_buyer_item_preview text := '';
BEGIN
  v_user_id := public.current_public_user_id();
  v_role_id := public.current_public_role_id();
  ...
  LOOP
    ...
    SELECT COALESCE(string_agg(seller_items.product_name, ', '), '')
    INTO v_seller_item_preview
    FROM (
      SELECT COALESCE(p.product_name, 'Product') AS product_name
      FROM public.order_items AS oi
      LEFT JOIN public.products AS p ON p.product_id = oi.product_id
    ...
    INSERT INTO public.notifications (user_id, order_id, title, message, notification_type, is_read, created_at)
    VALUES (
      seller_group.seller_id,
      v_order_id,
      'New Order Received',
      CASE
        WHEN COALESCE(v_seller_item_preview, '') = '' THEN format('A buyer placed sub-order %s.', v_reference || '-' || lpad(group_index::text, 2, '0'))
        ELSE format('A buyer placed sub-order %s. Items: %s', v_reference || '-' || lpad(group_index::text, 2, '0'), v_seller_item_preview)
      END,
      'system',
      0,
      NOW()
  ...
  SELECT COALESCE(string_agg(buyer_items.product_name, ', '), '')
  INTO v_buyer_item_preview
  FROM (
    SELECT COALESCE(p.product_name, 'Product') AS product_name
    FROM public.order_items AS oi
    LEFT JOIN public.products AS p ON p.product_id = oi.product_id
    WHERE oi.user_id = v_user_id
      AND oi.reference = v_reference
    ORDER BY oi.order_items_id
    LIMIT 3
  ) AS buyer_items;
  ...
  VALUES (
    ...
    'Order Placed',
    CASE
      WHEN COALESCE(v_buyer_item_preview, '') = '' THEN format('Your checkout order %s was placed successfully.', v_reference)
      ELSE format('Your checkout order %s was placed successfully. Items: %s', v_reference, v_buyer_item_preview)
    END,
    'order',
    0,
    NOW()
